...
> 개발 전에 아래 사항을 확정해야 합니다.

| # | 문서 | 불일치 | 해결 필요 |
|---|------|--------|-----------|
| 1 | 07-DB vs 12-CSV | reservations.status에 `cancelled`, `no_show`가 CSV에서 들어오지만 DB 설계에는 없음 | CHECK 제약에 추가 필요 |
| 2 | 05-플로우 | "반 밴드" (커뮤니티 게시판) 기능이 플로우차트에만 존재, 다른 문서에 없음 | P2로 분류 또는 삭제 확정 |
| 3 | 06-API vs 13-로드맵 | `/contracts`, `/counselings` 엔드포인트가 API 명세서에 없고 로드맵에만 있음 | API 명세서 업데이트 필요 |
| 4 | 07-DB | enrollments.total_sessions이 6, 7, 12인데 비즈니스 규칙이 불명확 | group=7, 1to1=6or12, director=? 확정 필요 |
